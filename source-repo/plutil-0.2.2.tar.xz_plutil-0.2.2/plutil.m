...
CFPropertyListRef readPropertyList(NSString *path)
{
	...
	if ([[NSFileManager defaultManager] fileExistsAtPath:jbroot(path)]) {
		NSData *plistData = [NSData dataWithContentsOfFile:jbroot(path)];
...
int main(int argc, const char **argv, const char **envp)
{
	...
	for (NSString *argument in dashedArgs) {
		...
		if ( ![argument caseInsensitiveCompare:@"-verbose"]
			...
			if ( ![[NSFileManager defaultManager] fileExistsAtPath:jbroot(dataPath)] ) {
				fprintf(
						stderr,
			...
			objectArg = [NSData dataWithContentsOfFile:jbroot(dataPath)];
			if ( !objectArg ) {
				fprintf(
						stderr,
			...
			for (NSString *path in filePaths) {
				...
				if ( plist ) {
					if ( plistFormat == 1 ) {
						...
						NSError *error;
						if ( ![json writeToFile:jbroot([[path stringByDeletingPathExtension] stringByAppendingPathExtension:@"json"]) atomically:YES encoding:4 error:&error])
